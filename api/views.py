...
def bookTrain(request, trainId, userId):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            booking = {"email": email, "mobile": mobile, "passengersCount": passengersCount, "journeyDate": journeyDate, "coachType": coachType,
                       "totalPrice": totalPrice, "user": user, "userId": userId, "trainName":trainName, "trainNumber": trainNumber, "StartStation": StartStation,
                       "destinationStation": destinationStation, "bookingStatus": bookingStatus}
        ...
        form = BookTrainForm()
    return render(request, 'user/bookTrain.html', {"train": train, "form": form, "success": success, "error": error})
